...
def onlineSpider(request):
    kw = request.GET.get("kw", "Java寮€鍙?")
    try:
        from .crawler_v2 import crawl_keyword_to_db

        file_name = f"zllog{int(time.time())}.txt"
        target_file = Path(settings.STATICFILES_DIRS[0]) / file_name
        crawl_keyword_to_db(kw, str(target_file))
        return HttpResponse(
            json.dumps({"msg": "success", "data": f"/{file_name}"}),
            content_type="application/json",
        )
    except Exception as exc:
        return HttpResponse(
            json.dumps({"msg": "false", "error": str(exc)}),
            content_type="application/json",
        )
...
def getJobsInfoByPageAndRows(page, rows, key_word):
    ...
    jobs = Job.objects.filter(
        Q(city__icontains=key_word["city"])
        & Q(job_experience__icontains=key_word["job_experience"])
        & Q(education__icontains=key_word["education"])
        & Q(post_type__icontains=key_word["post_type"])
    )
    paginator = Paginator(jobs, rows)
